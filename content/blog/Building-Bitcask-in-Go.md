...
The goal for my project was as follows: build a high performance concurrent, safe, and reliable implementation of the Bitcask database in Go, benchmark it, and document my learnings.  

It has been a long goal of mine to have a crystal clear understanding of concurrency and parallelism and in my view (at least right now) there is no better language to explore these concepts with, than Go. Go provides beautiful primives that allow me the programmer to easily create thousands of virtual threads, allow them to communicate via channels, and coordiante them with regular mutex primivies and select statements.
